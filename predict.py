...
# ── 4. 2위 모델 Registry 등록 ─────────────────────────────────
# Registry에 이미 등록된 run_id 목록 조회
already_registered = {
    v.run_id for v in client.search_model_versions("name='iris_classifier'")
}

# 미등록 run만 필터링
unregistered = [r for r in run_results if r["run_id"] not in already_registered]

if len(unregistered) == 0:
    print("⚠️ 등록할 새 모델이 없습니다.")
    exit(0)

# 미등록 중 가장 좋은 모델을 2위로 등록
second_best = unregistered[0]
print(f"\n🥈 등록할 모델: {second_best['run_name']} | accuracy: {second_best['accuracy']:.4f}")

registered = mlflow.register_model(
    model_uri=second_best["model_uri"],
    name="iris_classifier"
)
second_version = registered.version
print(f"✅ 등록 완료! Version: {second_version}")
